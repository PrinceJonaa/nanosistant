...
mod tests {
    ...
    fn normal_mode_supports_motion_and_insert_transition() {
        ...
        // move right with l
        editor.handle_normal_char(&mut session, 'l');
        assert_eq!(session.cursor, 1);

        // switch to insert with i
        editor.handle_normal_char(&mut session, 'i');
        assert_eq!(session.mode, EditorMode::Insert);
    }

    #[test]
    fn insert_char_appends_to_text() {
        let mut editor = LineEditor::new("> ", vec![]);
        let mut session = EditSession::new(false);

        editor.insert_active_char(&mut session, 'a');
        editor.insert_active_char(&mut session, 'b');
        editor.insert_active_char(&mut session, 'c');
    ...
    #[test]
    fn selection_bounds_works_for_forward_and_backward_selection() {
        let text = "hello";
        // forward selection: anchor=0, cursor=3
        assert_eq!(selection_bounds(text, 0, 3), Some((0, 4)));
        // backward selection: anchor=3, cursor=0
        assert_eq!(selection_bounds(text, 3, 0), Some((0, 4)));
    }

    #[test]
    fn ctrl_c_with_input_cancels_ctrl_c_without_exits() {
        let mut editor = LineEditor::new("> ", vec![]);
        let mut session_with_input = EditSession::new(false);
        session_with_input.text = "hello".to_string();
        session_with_input.cursor = 5;

        let key = KeyEvent::new(KeyCode::Char('c'), KeyModifiers::CONTROL);
        let action = editor.handle_key_event(&mut session_with_input, key);
        assert!(matches!(action, KeyAction::Cancel));

        let mut session_empty = EditSession::new(false);
        let key2 = KeyEvent::new(KeyCode::Char('c'), KeyModifiers::CONTROL);
        let action2 = editor.handle_key_event(&mut session_empty, key2);
        assert!(matches!(action2, KeyAction::Exit));
    }
}
