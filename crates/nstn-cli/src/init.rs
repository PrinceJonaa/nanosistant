...
const STARTER_NSTN_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let nstn_md = cwd.join("NANOSISTANT.md");
    let content = render_init_nanosistant_md(cwd);
    artifacts.push(InitArtifact {
        name: "NANOSISTANT.md",
        status: write_file_if_missing(&nstn_md, &content)?,
    });
...
pub(crate) fn render_init_nanosistant_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# NANOSISTANT.md".to_string(),
        String::new(),
        "This file provides guidance to Nanosistant when working with code in this repository."
            .to_string(),
        String::new(),
    ];
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_nanosistant_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("nstn-init-{nanos}"))
    }
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert_eq!(
            fs::read_to_string(root.join("NANOSISTANT.md")).expect("read existing nstn md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_nanosistant_md(Path::new(&root));
