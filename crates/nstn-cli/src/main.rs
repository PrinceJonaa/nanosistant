...
use nstn_plugins::{PluginManager, PluginManagerConfig};
...
use nstn_runtime::{
    ...
    OAuthCallbackParams, OAuthConfig, OAuthTokenExchangeRequest, OAuthTokenSet as RuntimeOAuthTokenSet,
    PermissionMode, PermissionPolicy, ProjectContext, RuntimeConfig, RuntimeFeatureConfig,
    RuntimeError, Session, TokenUsage, ToolError, ToolExecutor, UsageTracker,
    BootstrapPlan,
};
use serde_json::json;
use nstn_tools::GlobalToolRegistry;
...
Run `nanosistant --help` for usage."
        );
        std::process::exit(1);
    }
}
...
fn dump_manifests() {
    println!("nanosistant dump-manifests: manifest extraction not available in this build.");
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = config; // oauth config is hardcoded, not loaded from runtime config
    let callback_port = oauth.callback_port.unwrap_or(DEFAULT_OAUTH_CALLBACK_PORT);
    let redirect_uri = nstn_runtime::loopback_redirect_uri(callback_port);
    ...
    println!("Starting Nanosistant OAuth login...");
    ...
    save_oauth_credentials(&RuntimeOAuthTokenSet {
        access_token: token_set.access_token,
        refresh_token: token_set.refresh_token,
        expires_at: token_set.expires_at,
        scopes: token_set.scopes,
    })?;
    println!("Nanosistant OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("Nanosistant OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "Nanosistant OAuth login failed. You can close this window."
    } else {
        "Nanosistant OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Compact => {
            let result = nstn_runtime::compact_session(
                session,
                CompactionConfig {
        ...
        SlashCommand::Init => Ok(ResumeCommandOutcome {
            session: session.clone(),
            message: Some(init_nanosistant_md()?),
        }),
...
Type \x1b[1m/help\x1b[0m for commands\n\n\
  ...
  \x1b[2mSession\x1b[0m          {session_id}",
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("nanosistant-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("nanosistant-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("nanosistant-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No NANOSISTANT instruction files discovered in the current directory ancestry."
                .to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_nanosistant_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_nanosistant_md()?);
    Ok(())
}
...
) -> Result<(RuntimeFeatureConfig, GlobalToolRegistry), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let loader = ConfigLoader::default_for(&cwd);
    let runtime_config = loader.load()?;
    let plugin_manager = build_plugin_manager(&cwd, &loader, &runtime_config);
...
impl nstn_runtime::PermissionPrompter for CliPermissionPrompter {
    fn decide(
        &mut self,
        request: &nstn_runtime::PermissionRequest,
    ) -> nstn_runtime::PermissionPromptDecision {
        println!();
        ...
        match io::stdin().read_line(&mut response) {
            Ok(_) => {
                ...
                if matches!(normalized.as_str(), "y" | "yes") {
                    nstn_runtime::PermissionPromptDecision::Allow
                } else {
                    nstn_runtime::PermissionPromptDecision::Deny {
                        reason: format!(
            ...
            Err(error) => nstn_runtime::PermissionPromptDecision::Deny {
...
fn resolve_cli_auth_source() -> Result<AuthSource, Box<dyn std::error::Error>> {
    Ok(resolve_startup_auth_source(|| {
        // OAuth config is hardcoded (not stored in RuntimeConfig)
        Ok(Some(default_oauth_config()))
    })?)
}
...
fn final_assistant_text(summary: &nstn_runtime::TurnSummary) -> String {
    summary
        .assistant_messages
        .last()
        .map(|message| {
...
fn collect_tool_uses(summary: &nstn_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .assistant_messages
        .iter()
...
fn collect_tool_results(summary: &nstn_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .tool_results
        .iter()
        .flat_map(|message| message.blocks.iter())
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "nanosistant v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  nanosistant [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  nanosistant [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  nanosistant [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  nanosistant --resume SESSION.json [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  nanosistant dump-manifests")?;
    writeln!(out, "  nanosistant bootstrap-plan")?;
    writeln!(out, "  nanosistant agents")?;
    writeln!(out, "  nanosistant skills")?;
    writeln!(
        out,
        "  nanosistant system-prompt [--cwd PATH] [--date YYYY-MM-DD]"
    )?;
    writeln!(out, "  nanosistant login")?;
    writeln!(out, "  nanosistant logout")?;
    writeln!(out, "  nanosistant init")?;
    writeln!(out)?;
    writeln!(out, "Flags:")?;
    writeln!(
        out,
    ...
    writeln!(out, "  nanosistant --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  nanosistant --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  nanosistant --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  nanosistant --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  nanosistant agents")?;
    writeln!(out, "  nanosistant /skills")?;
    writeln!(out, "  nanosistant login")?;
...
mod tests {
    use super::{
        ...
        render_repl_help, response_to_events, resume_supported_slash_commands,
    ...
    use nstn_api::{MessageResponse, OutputContentBlock, Usage};
    use nstn_plugins::{PluginTool, PluginToolDefinition, PluginToolPermission};
    use nstn_runtime::{AssistantEvent, ContentBlock, ConversationMessage, MessageRole, PermissionMode};
    use serde_json::json;
    ...
    // re-export resolve_model_alias for test use
    fn resolve_model_alias(model: &str) -> String {
        super::resolve_model_alias_fn(model)
    }
    ...
    fn shared_help_uses_resume_annotation_copy() {
        let help = nstn_commands::render_slash_command_help();
    ...
    fn cost_report_uses_sectioned_layout() {
        let report = format_cost_report(nstn_runtime::TokenUsage {
            input_tokens: 20,
            output_tokens: 8,
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("nanosistant init"));
        assert!(help.contains("nanosistant agents"));
        assert!(help.contains("nanosistant skills"));
        assert!(help.contains("nanosistant /skills"));
    }
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            StatusUsage {
                message_count: 7,
                turns: 3,
                latest: nstn_runtime::TokenUsage {
                    input_tokens: 5,
                    output_tokens: 4,
                    cache_creation_input_tokens: 1,
                    cache_read_input_tokens: 0,
                },
                cumulative: nstn_runtime::TokenUsage {
                    input_tokens: 20,
                    output_tokens: 8,
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_nanosistant_md(std::path::Path::new("."));
        assert!(rendered.contains("# NANOSISTANT.md"));
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p nstn-cli"}"#)
                .contains("cargo test -p nstn-cli")
        );
        assert_eq!(
...
#[allow(dead_code)]
fn resolve_model_alias_fn(model: &str) -> String {
    nstn_api::resolve_model_alias(model)
}
