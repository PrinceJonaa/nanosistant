...
const GITHUB_CLIENT_ID = 'Ov23liM7YNgwspB2kBe3';
...
const AUTH = {
  ...
  login() {
    // Build OAuth URL directly with the public client ID (secret stays server-side)
    const redirect = encodeURIComponent('https://princejonaa.github.io/nanosistant/');
    const state = Date.now().toString(36);
    sessionStorage.setItem('oauth_state', state);
    window.location.href = `https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&redirect_uri=${redirect}&scope=public_repo&state=${state}`;
  },
